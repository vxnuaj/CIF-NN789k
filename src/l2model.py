...
def cce(mini_onehot, a2, w1, lambd):
    eps = 1e-8 
    fro_norm_w1 = (lambd * np.sum(np.square(w1))) / mini_onehot.shape[1] 
    loss = - ( 1 / mini_onehot.shape[1] ) * np.sum(mini_onehot * np.log(a2 + eps))
    reg_loss = loss + fro_norm_w1 
    return reg_loss
...
def backward(x, mini_onehot, w2, w1, a2, a1, z2_norm, z1_bnorm, z1_norm, g2, g1, std2, std1, lambd):
    eps = 1e-8
    ...
    dw1 = (np.dot(dz1, x.T) + (2 * lambd * w1)) / mini_onehot.shape[1]
...
def gradient_descent(x, y, w1, b1, g1, w2, b2, g2, ewa_mu1, ewa_var1, ewa_mu2, ewa_var2, alpha, beta_1, beta_2, lambd, epochs, file, file2): 
    ...
    for epoch in range(epochs):
        ...
        for i in range(x.shape[0]):
            ...
            z1_norm, z1_bnorm, std1, var1, mu1, a1, z2_norm, z2_bnorm, std2, var2, mu2, a2  = forward(x[i], w1, b1, g1, w2, b2, g2, )
            
            loss = cce(one_hot_y[i], a2, w1, lambd)
            acc, pred = accuracy(y[i], a2) 
            
            dw1, db1, dg1, dw2, db2, dg2 = backward(x[i], one_hot_y[i], w2, w1, a2, a1, z2_norm, z1_bnorm, z1_norm, g2, g1, std2, std1, lambd)
...
def l2model(x, y, input_features, hidden_sizes, output_size, alpha, beta_1, beta_2, lambd, epochs, file, file2): 
    try:
    ...
    w1, b1, g1, w2, b2, g2, epochs_vec, loss_vec, acc_vec, itr_vec, ewa_mu1, ewa_var1, ewa_mu2, ewa_var2 = gradient_descent(x, y, w1, b1, g1, w2, b2, g2, ewa_mu1, ewa_var1, ewa_mu2, ewa_var2, alpha, beta_1, beta_2, lambd, epochs, file, file2)
